...
# Thêm thư mục song song vào Python path
...
def train_model(model, data1: list, data2: list, epoch_num: int, batch_size: int):
    ...
    checkpointer = ModelCheckpoint(
        filepath=os.path.join(os.getcwd(), "examples", "temp_weights(1).weights.h5"),
        verbose=1,
        save_best_only=True,
    ...
    model.load_weights(os.path.join(os.getcwd(), "examples", "temp_weights(1).weights.h5"))
...
def test_model(model, data1: list, data2: list, outdim_size: int, apply_kcca: bool):
    ...
    if kcca:
        ...
        m = [None, None]
        print("KCCA started!")
        w[0], w[1], m[0], m[1] = kcca(new_data[0][0], new_data[0][1], outdim_size)
        print("KCCA ended!")
...
if __name__ == "__main__":
    ...
    model, history = train_model(model, data1, data2, epoch_num, batch_size)
    new_data = np.array(test_model(model, data1, data2, outdim_size, apply_kcca), dtype="object")
    ...
    save_to = os.path.join(os.getcwd(), "examples", "new_features(1).npy")
